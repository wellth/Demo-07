...
/*
	...
	start_timer = 0;
*/
system("lp -o landscape -o Quality=600GrayscaleK -o scaling=77 plot/anno_ecgprint.png");
}
...
void show_window_health_summary()
{
	...
	strcpy(BMI_img_path,"GraphsNValues/blank.png");
	strcpy(SBP_img_path,"GraphsNValues/blank.png");
	strcpy(DBP_img_path,"GraphsNValues/blank.png");
	strcpy(BF_img_path,"GraphsNValues/blank.png");
	...
	if( (UserSummaryData.BMI_val == 0) || (UserSummaryData.Age == 0) || (UserSummaryData.Gender == NULL) ){
		printf("BMI - %f Age - %d Gender - %s \n",UserSummaryData.BMI_val,UserSummaryData.Age,UserSummaryData.Gender);
		strcpy(graph_wt_buf , "Test Was Skipped");
		gtk_label_set_label (grph_wt_label, graph_wt_buf);
		GdkPixbufAnimation * pixbufBMI = gdk_pixbuf_animation_new_from_file("GraphsNValues/blank.png", error);
...
/**********************************Body Fat test************************************/
	...
	if( (UserSummaryData.BMI_val ==0) || (UserSummaryData.BF_val == 0) || (UserSummaryData.Age == 0) || (UserSummaryData.Gender == NULL) ){
		...
		GdkPixbufAnimation * pixbufBF = gdk_pixbuf_animation_new_from_file("GraphsNValues/blank.png", error);
...
/**********************************BP test************************************/
	...
	if( (UserSummaryData.SysBP == 0) || (UserSummaryData.DiaBp == 0) || (UserSummaryData.Age == 0) || (UserSummaryData.Gender == NULL) ){
		...
		GdkPixbufAnimation * pixbufSBP = gdk_pixbuf_animation_new_from_file("GraphsNValues/blank.png", error);
		GdkPixbufAnimation * pixbufDBP = gdk_pixbuf_animation_new_from_file("GraphsNValues/blank.png", error);
...
//sprintf(SysStr,"convert plot/ecgprint.png -gravity East -background white -splice 260x0 -annotate +0+2 '*******************************************\n Thanks for using the Wellth Station \n\nCell No: %s   Age : %d yrs\nGender: %s   Height: %.0f cms\n\nWeight: %.2f kgs   BMI: %.2f \n\n BodyFat : %.2f \% \n\nSBP: %.0f mmHg    DBP: %.0f mmHg \n For Rs. 50, get a clinical interpretation of \nyour ECG mailed home within 72 hours \n\n All these tests [clinically validated] are \nfor your informational purposes only.\nThese tests are NOT a \n replacement for medical advice.\nBlood pressure readings are dynamic and \n need to be observed at\nleast over 3 different days.\nPlease consult your doctor OR any \nqualified healthcare provider for\nclinical interpretation of results.\n\nEmail info@wellth.in OR call 919945585990 \nfor assistance with your ECG chart or \n more information on other tests.\n\n******************************\n' plot/anno_ecgprint.png",UserSummaryData.Login_name,UserSummaryData.Age,UserSummaryData.Gender, UserSummaryData.Height,UserSummaryData.Weight,UserSummaryData.BMI_val,UserSummaryData.BF_val,UserSummaryData.SysBP,UserSummaryData.DiaBp);
     Total_Health_Score = Bmi_Health_Score + Bp_Sys_Health_Score + Bp_Dia_Health_Score + Bf_Health_Score ;
	switch (Total_Health_Score)
	{
	case 1 :
sprintf(SysStr,"convert plot/ecgprint.png -gravity North -background white -pointsize 16 -splice 0x80 -annotate +0+2 ' * \n Cell No: %s   Age : %d yrs Gender: %s  \n Height: %.0f cms  Weight: %.2f kgs  BMI: %.2f \n BodyFat : %.2f \% \n SBP: %.0f mmHg    DBP: %.0f mmHg \n\n ' plot/anno_ecgprint.png",UserSummaryData.Login_name,UserSummaryData.Age,UserSummaryData.Gender, UserSummaryData.Height,UserSummaryData.Weight,UserSummaryData.BMI_val,UserSummaryData.BF_val,UserSummaryData.SysBP,UserSummaryData.DiaBp);
		break;
	case 2 :
sprintf(SysStr,"convert plot/ecgprint.png -gravity North -background white -pointsize 16 -splice 0x80 -annotate +0+2 ' ** \n Cell No: %s   Age : %d yrs Gender: %s  \n Height: %.0f cms  Weight: %.2f kgs  BMI: %.2f \n BodyFat : %.2f \% \n SBP: %.0f mmHg    DBP: %.0f mmHg \n\n ' plot/anno_ecgprint.png",UserSummaryData.Login_name,UserSummaryData.Age,UserSummaryData.Gender, UserSummaryData.Height,UserSummaryData.Weight,UserSummaryData.BMI_val,UserSummaryData.BF_val,UserSummaryData.SysBP,UserSummaryData.DiaBp);
		break;
...
sprintf(SysStr,"convert plot/ecgprint.png -gravity North -background white -pointsize 16 -splice 0x80 -annotate +0+2 ' **** \n Cell No: %s   Age : %d yrs Gender: %s  \n Height: %.0f cms  Weight: %.2f kgs  BMI: %.2f \n BodyFat : %.2f \% \n SBP: %.0f mmHg    DBP: %.0f mmHg \n\n ' plot/anno_ecgprint.png",UserSummaryData.Login_name,UserSummaryData.Age,UserSummaryData.Gender, UserSummaryData.Height,UserSummaryData.Weight,UserSummaryData.BMI_val,UserSummaryData.BF_val,UserSummaryData.SysBP,UserSummaryData.DiaBp);
		break;
	default :
sprintf(SysStr,"convert plot/ecgprint.png -gravity North -background white -pointsize 16 -splice 0x80 -annotate +0+2 'Cell No: %s   Age : %d yrs Gender: %s  \n Height: %.0f cms  Weight: %.2f kgs  BMI: %.2f \n BodyFat : %.2f \% \n SBP: %.0f mmHg    DBP: %.0f mmHg \n\n ' plot/anno_ecgprint.png",UserSummaryData.Login_name,UserSummaryData.Age,UserSummaryData.Gender, UserSummaryData.Height,UserSummaryData.Weight,UserSummaryData.BMI_val,UserSummaryData.BF_val,UserSummaryData.SysBP,UserSummaryData.DiaBp);
		break;
	}
system(SysStr);
system("lp -o landscape -o Quality=600GrayscaleK -o scaling=77 plot/anno_ecgprint.png");
